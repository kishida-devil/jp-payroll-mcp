...
<p class="lede">47都道府県の保険料率、源泉所得税、標準報酬月額の決定と改定、割増賃金、年次有給休暇、最低賃金、令和8年分の年末調整、住民税の見込み額まで。<strong>答えには根拠の条文または通知が付きます。</strong></p>

<p><span class="tag">無料枠あり</span><span class="tag">APIキー不要</span><span class="tag">MCPツール 30本</span><span class="tag">エンドポイント 45本</span><span class="tag">OpenAPI 3.0</span></p>
...
<li><strong>住民税は「見込み額」です。</strong>前年の所得と市区町村から地方税法の標準税率と各自治体の超過課税で計算し、横浜市・名古屋市の公表計算例と一致しますが、決定額は市区町村の特別徴収税額通知書で、給与からはその額を控除します。</li>
<li><strong>国民健康保険の額も返しません。</strong>市町村の条例で決まるため、全国一律の額が存在しません。</li>
<li><strong>年末調整は令和8年分の計算を返します。</strong>医療費控除・寄附金控除・雑損控除は法律上は年末調整で引けないので、渡されたときは「確定申告をした場合の見込み」として別枠で返します。申告書に書かれた内容をそのまま渡してください。配偶者や扶養親族を推定しません。</li>
