...
公表されている料額表・税額表と1セルずつ突き合わせ、変更のたびに **4,440件** の検証を実行します。
...
Verified against the published tables cell by cell: **4,440 assertions** on every change.
...
`test/verify.mjs` runs 4,440 assertions against a live server. The core of it compares the
