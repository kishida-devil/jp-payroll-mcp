...
RECIPE = {
    ...
    #
    # 買い手は日本人なので日本語を先に置く。ただし RapidAPI 内の検索は英語でも効くので、
    # `Japan payroll` `social insurance` は残す。一覧に出る一行で、ここで読まれなければ
    # 中を見てもらえない。
    "short_description": (
        "日本の給与計算・社会保険をひとつのAPIに。47都道府県の保険料率、源泉徴収税額表、"
    ...
    "long_description": (
        "日本の給与計算に必要な数字は公開されているのに、47都道府県ぶんの表、"
        "毎年4月に変わる省庁のPDF、10月に都道府県ごと別々の日付で改定される"
        "最低賃金のサイトに散らばっています。\n\n"
        "このAPIはそれをまとめます。保険料は給与額ではなく標準報酬月額"
        "(50等級の階段)にかかり、同じ等級内の昇給では変わりません。"
        "雇用保険だけは実際の賃金にかかります。従業員負担分の端数は"
        "0.50円以下切り捨て・超過切り上げで、四捨五入ではありません。\n\n"
        "判定系のエンドポイントは結論だけでなく根拠を返します。"
        "随時改定の「2等級以上」は健康保険法ではなく昭和36年保発第4号にあり、"
        "上下限付近には1等級で足りる例外が4つあります。健康保険と厚生年金は"
        "別々に判定するので、昇給が片方だけ動かすことがよくあります。\n\n"
        "資格喪失は退職日の翌日です(健康保険法第36条)。3月31日退職なら喪失は"
        "4月1日で3月分の保険料がかかり、3月30日退職ならかかりません。"
        "東京・40歳・月給30万円で労使合計95,130円の差が1日で生まれます。\n\n"
        "年齢は誕生日の前日に上がります(年齢計算ニ関スル法律)。"
        "4月1日生まれの人の介護保険料は3月分から引きます。\n\n"
        "すべての数値は公式資料から機械的に抽出し、そこに印刷された値と"
        "突き合わせています。協会けんぽの保険料額表と250通りの都道府県×等級、"
        "国税庁の月額表の公表2,079セル全部。変更のたびに全件を実行します。\n\n"
        "答えには根拠の条文が付き、?include=statute_text でその本文まで取れます。"
        "収録範囲の外の日付は、古い数字を返さずに422で断ります。"
        ...
        "Every figure is computed from the published government tables and carries "
        "the statute or ministerial notice it rests on."
    ),
