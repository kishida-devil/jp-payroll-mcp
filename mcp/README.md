...
営業日計算、令和8年分の年末調整、住民税の見込み額まで30のツールを提供します。
...
**English:** Japanese payroll, social insurance and labour law as an MCP server — 30 tools, free, no key. See **[README.en.md](https://github.com/kishida-devil/jp-payroll-mcp/blob/main/mcp/README.en.md)**.
...
| `calculate_year_end_adjustment` | 年末調整(令和8年分)。源泉徴収簿の⑦〜㉗を全部返す。国税庁の設例と一致。医療費・寄附金・雑損は「確定申告をしたら」の別枠 |
| `estimate_resident_tax` | 住民税の見込み額。前年所得と自治体から所得割・均等割・調整控除・ふるさと納税・非課税判定まで。横浜市・名古屋市の公表計算例と一致 |
...
- **住民税は見込み額を返します(`estimate_resident_tax`)。** 決定額は市区町村が特別徴収税額通知書で決めるもので、
  給与からはその額を控除します。見込み額は横浜市・名古屋市の公表計算例と一致しますが、通知書の代わりではありません。
- **年末調整は令和8年分の計算に対応しています。**医療費控除・寄附金控除・雑損控除は法律上は年末調整で引けないので、渡されたときは「確定申告をした場合の見込み」として別枠で返します。申告書に書かれた内容だけを控除し、配偶者や扶養親族を推定しません。
