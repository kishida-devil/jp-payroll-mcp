...
営業日計算、令和8年分の年末調整まで29のツールを提供します。
...
**English:** Japanese payroll, social insurance and labour law as an MCP server — 29 tools, free, no key. See **[README.en.md](https://github.com/kishida-devil/jp-payroll-mcp/blob/main/mcp/README.en.md)**.
...
| `calculate_year_end_adjustment` | 年末調整(令和8年分)。源泉徴収簿の⑦〜㉗を全部返す。国税庁の設例と一致 |
...
背後のAPIは変更のたびに **4,661件** の検証を実行します。中核は、協会けんぽの
...
- **年末調整は令和8年分の計算に対応しています。**医療費控除・寄附金控除・雑損控除は年末調整の対象外(確定申告)です。申告書に書かれた内容だけを控除し、配偶者や扶養親族を推定しません。
